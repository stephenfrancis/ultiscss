...
export default class Project {
  ...
  private collectTemplateUIComponents(
    template_id: string,
    collector: string[]
  ): void {
  ...
  public convertEJSFile(filename: string): Promise<string> {
    const html_file: string = filename
      .replace(this.source_dir, this.target_dir)
      .replace(/\.ejs$/, ".html");
  ...
  public generateSCSSFileFromReferences(
    components: string[],
    target_file: string
  ): void {
    const content =
  ...
  private getObjectDataInternal(object_id: string): any {
    const parts = Utils.getPartsFromObjectId(object_id);
    const defn_file: string =
      this.source_dir + "/" + parts.namespace + "/" + object_id + ".ejs";
    ...
    if (out.references.indexOf(object_id) > -1) {
  ...
  public getSignature(component_id: string): Signature {
    if (!this.sig_map[component_id]) {
      ...
      const defn_file: string =
        this.source_dir + "/" + parts.namespace + "/" + component_id + ".ejs";
      this.makeNewSignature(
        component_id,
        parts.namespace,
        Utils.loadFile(defn_file)
      );
    }
  ...
  public loadSourceReferences(): void {
    ...
    Object.keys(this.collection).forEach((ref_from) => {
      ...
      const data = JSON.parse(
        Utils.loadFile(
          this.target_dir + "/" + parts.namespace + "/" + ref_from + ".json"
        )
      );
...
*/
  ...
  private makeNewSignature(
    component_id: string,
    namespace: string,
    ejs_data: string
  ): void {
    const root_div = Utils.getRootElement(Utils.getCheerio(ejs_data));
    this.sig_map[component_id] = Signature.createFromCheerioNode(
      namespace,
      component_id,
      root_div
    );
  ...
  public processAggregate(filename: string, reporter: Reporter): string {
    try {
      const root_div = Utils.getRootElement(
        Utils.getCheerio(Utils.loadFile(filename))
      );
  ...
  private reportUnusedObjects(): string[] {
    ...
    Object.keys(this.collection).forEach((key: string) => {
      if (key.charAt(0) === "a" || key === this.entry_point_template) {
        // ignore aggregates
        return;
      }
      ...
      this.collection[key].forEachReferencedBy((ref_from: string) => {
        ref_found =
          ref_found || (key !== ref_from && ref_from.charAt(0) === "s");
      });
      if (!ref_found) {
        out.push(key);
      }
  ...
  public testMarkup(
    markup: cheerio.Cheerio | string,
    reporter: Reporter
  ): void {
    if (typeof markup === "string") {
      markup = Utils.getRootElement(Utils.getCheerio(markup));
    }
    Utils.drillDownMarkup(markup, "*", (child: cheerio.Cheerio, data: any) => {
      Utils.splitClasses(child.attr("class")).forEach((class_name) => {
        ...
        try {
          ...
          reporter.info(
            `checking class ${class_name} at position ${data.position} against found signature`
          );
          signature.testMarkup(child, reporter);
        } catch (e) {
          if (
            e.toString().indexOf("ENOENT: no such file or directory, open") > 0
          ) {
  ...
  public toString(): string {
    return (
      `ultiscss ${this.ultiscss_version}: project ${this.project_name}:${this.project_version}` +
