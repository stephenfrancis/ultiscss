...
export default class Signature {
  private cheerio_node?: cheerio.Cheerio;
  ...
  constructor(
    ...
    component_id: string,
    title: string,
    hide_in_gallery: boolean,
    leniency_level: string
  ) {
    if (!namespace || !component_id) {
      throw new Error(
        `namespace ${namespace} and component_id ${component_id} must be non-blank`
      );
    }
    ...
    this.leniency_level = decodeLeniencyLevel(
      leniency_level
    ) as Types.LeniencyType[];
    ...
    if (component_id.indexOf(namespace) !== 2) {
      throw new Error(
        `component id: ${component_id} doesn't incorporate namespace: ${namespace}`
      );
    }
  }
  ...
  public static createFromCheerioNode(
    ...
    Utils.drillDownMarkup(
      cheerio_node,
      "*",
      (child: cheerio.Cheerio, data: any) => {
        if (sig && !sig.includeElement(child)) {
          return;
        }
        const node = Node.createFromCheerioNode(
          child,
          parent_node[data.level],
          sig
        );
        if (data.level === 0) {
  ...
  public includeElement(cheerio_node: cheerio.Cheerio): boolean {
    // return (node && node[0] && node[0].name === "div");
    return (
      !this.isLayout() || (cheerio_node[0] as cheerio.TagElement).name === "div"
    );
  }
  ...
  public testMarkup(div: cheerio.Cheerio, reporter: Reporter): void {
    if (!this.root_node) {
  ...
  public validate(reporter: Reporter): void {
    ...
    if (!this.root_node.containsRequiredClass(this.component_id)) {
      reporter.error(
        `top level element must have ${this.component_id} as a required class`
      );
    }
    if (this.isLayout()) {
  ...
  public validateMixedContent(reporter: Reporter): void {
    ...
    this.cheerio_node.children().each((index, child) => {
      if ((child as cheerio.TagElement).name === "div") {
        contains_divs = true;
      } else {
    ...
    try {
      ...
      Utils.forEachClassInCSS(css_ast, (class_name) => {
        if (!Utils.testClassMatches(classes, class_name)) {
          reporter[this.isLenient("classes") ? "warn" : "error"](
            `unrecognized class for signature: ${class_name}`
          );
        }
      });
      if (this.isLayout()) {
        Utils.forEachPropertyInCSS(css_ast, (property_name) => {
          if (layout_forbidden_css.indexOf(property_name) > -1) {
            reporter[this.isLenient("css-decls") ? "warn" : "error"](
              `invalid CSS declaration for layout: ${property_name}`
            );
          }
        });
      }
