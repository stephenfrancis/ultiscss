...
test("all signature", () => {
  ...
  const new_sig = Signature.createFromCheerioNode(
    "test",
    "l-test-main",
    makeNode(node1, "div")
  );
  ...
  expect(new_sig.getRootNode().toString()).toBe(
    "<div.l-test-main.some-optional? />"
  );
  ...
  expect(rep.get(0, "WARN")).toEqual({
    level: "warn",
    msg: "l-test-main/0: class name is not valid: some-optional",
  });
  ...
  new_sig.testMarkup(
    makeNode(
      {
        // successful - optional class not present
        class: "l-test-main",
      },
      "div"
    ),
  ...
  new_sig.testMarkup(
    makeNode(
      {
        // successful - optional class present
        class: "l-test-main some-optional",
      },
      "div"
    ),
  ...
  expect(rep.get(0, "ERROR")).toEqual({
    level: "error",
    msg: "l-test-main/0: class not allowed: blah",
  });
  expect(rep.get(1, "ERROR")).toEqual({
    level: "error",
    msg: "l-test-main/0: required class(es) not present: l-test-main",
  });
  ...
  expect(rep.get(0, "ERROR")).toEqual({
    level: "error",
    msg: "unrecognized class for signature: l-blah",
  });
  ...
  expect(rep.get(0, "ERROR")).toEqual({
    level: "error",
    msg: "invalid CSS declaration for layout: text-decoration",
  });
});

test("deep signature", () => {
  const div = Lib.getRootElement(
    Lib.getCheerio(Lib.loadFile("src/scss/demo/l-demo-main.ejs"))
  );
...
test("class patterns", () => {
  let div = Lib.getRootElement(
    Lib.getCheerio(Lib.loadFile("src/scss/demo/w-demo-pattern.ejs"))
  );
  ...
  sig.testMarkup(
    Lib.getRootElement(
      Lib.getCheerio(
        Lib.loadFile("src/__tests__/examples/class_pattern_1.html")
      )
    ),
    rep
  );
  ...
  sig.testMarkup(
    Lib.getRootElement(
      Lib.getCheerio(
        Lib.loadFile("src/__tests__/examples/class_pattern_2.html")
      )
    ),
    rep
  );
  ...
  sig.testMarkup(
    Lib.getRootElement(
      Lib.getCheerio(
        Lib.loadFile("src/__tests__/examples/class_pattern_3.html")
      )
    ),
    rep
  );
  ...
  expect(rep.get(0, "ERROR")).toEqual({
    level: "error",
    msg: "w-demo-pattern/0.0: class not allowed: fb-bobbins",
  });
  expect(rep.get(1, "ERROR")).toEqual({
    level: "error",
    msg: "w-demo-pattern/0.0: required class(es) not present: fa-*",
  });
  ...
  sig.testMarkup(
    Lib.getRootElement(
      Lib.getCheerio(
        Lib.loadFile("src/__tests__/examples/class_pattern_4.html")
      )
    ),
    rep
  );
  ...
  div = Lib.getRootElement(
    Lib.getCheerio(Lib.loadFile("src/scss/demo/w-demo-pattern-2.ejs"))
  );
